...
export class FormInstance {
	public readonly metadata: server.FormMetadata;
	public outputs: OutputFieldValue[] = [];
	public inputs: Array<InputController<any>> = [];

	constructor(metadata: server.FormMetadata, controlRegister: ControlRegister) {
		this.metadata = new server.FormMetadata(metadata);
	...
	public async submit(app: UmfApp, asPostOnLoad: boolean, args: any): Promise<server.FormResponse> {
		if (asPostOnLoad) {
			this.enforceCanPostOnLoad();
		}
	...
	public static getOutputFieldValues(outputFieldsMetadata: server.OutputFieldMetadata[], response: any): OutputFieldValue[] {
	...
	public setOutputFieldValues(response: server.FormResponse): void {
		if (response == null) {
			this.outputs = [];
			return;
		}
	...
	public async handleEvent(eventName: string, eventMetadata: server.EventHandlerMetadata, parameters: FormEventArguments): Promise<void> {
	...
	private static getNormalizedObject(response: server.FormResponse): any {
