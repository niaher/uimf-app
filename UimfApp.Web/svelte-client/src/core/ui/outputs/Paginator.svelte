...
<script>
	...
	let container;
	let pageSize;
	$: pages = getPages(field, form, parent);
	...
	function goToPage(page) {
		parent.form.setInputFields(page.params);
		parent.submit(null, false);
	}

	function changePageSize() {
		const paginatorInput = parent.form.inputs.find(t => t.metadata.id === field.metadata.customProperties.customizations.paginator);
		paginatorInput.value.pageSize = field.pageSize;
		paginatorInput.value.pageIndex = 1;

		const params = {};
		for (const i of parent.form.inputs) {
	...
	function getPages (field, form, parent) {
		const paginatorInput = form.inputs.find(t => t.metadata.id === field.metadata.customProperties.customizations.paginator);
